...
- `allow_major_updates` (`entities.MajorUpdatesAllowed`, unset = **true**) decides whether an upgrade may cross a major. Accepted from any layer in **both** directions, including the target repo's `.autoupdate.yaml` — it starts nothing and destroys nothing. Reaches the updaters via `UpdateOptions.AllowMajorUpdates`, whose zero value is the restrictive case, so build it through the run command. `java` passes `-DallowMajorUpdates`; `golang` emits `support.GoMajorGuardScript()` only when false (`support.GoCompileGuardScript()` is emitted either way). On by default because a held-back dependency never gets its CVEs remediated and the pipeline is what catches a breaking major. Does **not** relax the pre-release filter
...
- `support.GoMajorAvailableScript()` reports the majors `go get -u` structurally cannot reach: v2+ live behind a `/vN` path suffix, which is a *different module*, so `-u` never takes one and the repository looks up to date forever. It probes `go list -m -versions <path>/v<next>` for each **direct** requirement (indirect ones are the graph's business), walking up until a miss, deciding existence on the **version list and not the exit status** — that command answers `0` with an empty list for a major that was never published. `gopkg.in/...vN` paths are skipped, not probed. Hits become `GO_MAJOR_AVAILABLE=` markers that `golang.ParseMajorsAvailable` turns into a table in the PR description, above the review checklist; with no hits the section is absent entirely. It reports and never applies: crossing a major means rewriting imports, which is a migration somebody decides on.
- Not every break is visible in a version string. `support.GoCompileGuardScript()` is emitted by `golang` **unconditionally** (unlike the major guard) and verifies the upgraded module still type-checks, because `go get -u` raises indirect requirements past what the graph asks for and an untagged module's pseudo-version moves no major, no minor and nothing a comparison can catch. It asks `go vet ./...`, never `go build ./...`: a module with no non-test Go files builds clean while its test packages fail. On a regression it holds back the indirect requirements the run raised (`go mod edit -require`, which sets a floor and so preserves a deliberate pin, never `-droprequire`), re-tidies and re-checks. **It never reverts an upgrade**: if holding is not enough the cause is almost always a *direct* dependency, which is news, so the upgrade is left as written and the guard returns non-zero — a red pull request is the report. Reverting would leave an empty branch and, in a single-module repository, no pull request at all, swallowing a breaking upgrade on every run. A module that already failed the check before the upgrade also keeps its upgrade — the comparison is against the state the run started from, not against "clean", so a pre-existing vet diagnostic cannot freeze indirect deps forever.
