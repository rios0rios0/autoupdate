...
- `allow_major_updates` (`entities.MajorUpdatesAllowed`, unset = **true**) decides whether an upgrade may cross a major. Accepted from any layer in **both** directions, including the target repo's `.autoupdate.yaml` — it starts nothing and destroys nothing. Reaches the updaters via `UpdateOptions.AllowMajorUpdates`, whose zero value is the restrictive case, so build it through the run command. `java` passes `-DallowMajorUpdates`; `golang` emits `support.GoMajorGuardScript()` only when false. On by default because a held-back dependency never gets its CVEs remediated and the pipeline is what catches a breaking major. Does **not** relax the pre-release filter
