...
| `allow_major_updates` | Either direction. On by default; set `false` to hold this repository on its current major line. Honoured by `java`, `golang`, `dart`, `terraform`, `pipeline`, `csharp` and `ruby`. `javascript`, `python` and the gem half of `ruby` delegate to a package manager that already resolves inside the declared ranges, so it has nothing to gate there; `dockerfile` refuses majors unconditionally and does not read it yet |
