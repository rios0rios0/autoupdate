...
- added `.fvmrc` SDK bumping, the Flutter counterpart of `.ruby-version` and `.nvmrc`. It is rewritten in Go rather than in the generated bash because `.fvmrc` is JSON and a `flavors` block does not survive being edited with `grep` and `sed`. `environment: sdk:` in `pubspec.yaml` is deliberately left alone: raising a package's SDK floor is a compatibility decision for its consumers, and `pub upgrade --major-versions` already raises it when a dependency actually requires it
