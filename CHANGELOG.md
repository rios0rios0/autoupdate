...
- taught the Ruby updater to raise Gemfile constraints past a major so `allow_major_updates` can actually resolve one. Bundler has no flag meaning `raise the bounds in the Gemfile` -- `bundle update` resolves inside whatever is declared and `--major` only lifts bundler's own ceiling -- so `gem "rails", "~> 6.0"` stayed on 6.x for ever however the key was set, while every other ecosystem had a native answer: `pub upgrade --major-versions`, `pdm --unconstrained`, `pnpm update --latest`, `npm-check-updates`. The rewrite works in two moves around `bundle update`: every pessimistic (`~>`) constraint on a `gem` line is widened to `>=` before the resolution so bundler may cross the major, and re-tightened afterwards to `~>` the version that resolved, at the precision the repository wrote -- `~> 6.0` becomes `~> 7.1`, and a constraint whose gem stayed inside its bound is put back untouched; `bundle lock` then reconciles the lockfile, whose `DEPENDENCIES` block records the widened requirements the resolution ran against and which a frozen install would otherwise refuse. The manifest therefore keeps stating a bound, which is the same contract `pub upgrade --major-versions` and `npm-check-updates` honour, and turning the key off again holds the repository where it is. When the widened resolution fails the manifest is restored and `bundle update` retried within the declared bounds, so a conflict degrades to the smaller upgrade rather than to a dropped ceiling beside a stale lockfile. Exact pins and explicit `<` bounds are left alone, being the repository saying no in as many words; the `ruby` directive, commented-out declarations and bundler plugins are never touched, because the rewrite is anchored on the `gem` call rather than on the operator; and only the `Gemfile` is rewritten, never a `.gemspec`, whose constraints describe what consumers of the library must tolerate rather than what this application may resolve to. The generated pull request description names the raised constraints and asks the reviewer to check them.

### Changed

- changed the Go module dependencies to their latest versions
- completed `allow_major_updates` across every updater. `dockerfile` refused every major unconditionally, which made it the only one whose answer no configuration could change -- a Dockerfile on `node:20` stayed on 20 for ever, including past its end of life. With majors allowed its two version-distance rules now fall away together, because crossing a major implies crossing a minor and keeping the patch-pin rule would leave a three-part pin unable to move at all; suffix and precision still have to match, since those encode what the Dockerfile chose rather than how far behind it is. `javascript`, `python` and the gem half of `ruby` could not cross a major at all: `npm update`, `pnpm update`, `yarn upgrade`, `pdm update` and `bundle update` all resolve inside the ranges the manifest already declares, so the policy of taking the newest release was never delivered for them. Each needed a different mechanism -- `pnpm update --latest`, `pdm update --unconstrained`, `npm-check-updates` fetched through `npx` with a fallback to the plain update when it cannot be reached, and for yarn a run-time choice between Berry's `up` and Classic's `upgrade --latest` because nothing on the Go side knows which is installed. Ruby is the asymmetric one: bundler has no flag meaning `raise the Gemfile bounds`, and rewriting a `~>` is a different act from resolving within it, so only the refusing direction is expressed -- `bundle update --minor` caps the bump that plain `bundle update` would otherwise take on an unconstrained gem, which is the direction that was actually missing.
- made the major-version guard configurable and turned it off by default. `allow_major_updates` is settable in any layer, including the target repository's own `.autoupdate.yaml`, and an unset value now means majors are taken. The guard shipped unconditional, which had it backwards: a dependency held back is a dependency whose CVEs are never remediated, and the fix for a serious one is often only in the next major -- Spring Framework 6.2.x carried six unpatched advisories, four of them 9.8, whose only remedy was 7.0.x, and no 6.2 release ever shipped one. Refusing the major there does not avoid risk, it accumulates it silently. What catches a breaking change instead is the pipeline the pull request already runs: an incompatible major arrives as a red check on a branch rather than as a broken main, which is a more accurate signal than a version-number heuristic that cannot tell a rename from a rewrite. `java` passes the value through as `-DallowMajorUpdates` on both versions-maven-plugin goals; `golang` emits the guard only when it is false, so the generated script carries no bash function it will not call. Unlike `cleanup_stale_branches` the key is accepted in both directions from a repository's own file: it starts nothing and destroys nothing, and a repository declining majors is declining upgrades for itself. The pre-release filter is unchanged and unaffected -- a beta is never a candidate, whatever this is set to, because taking one is how CVEs get reintroduced rather than remediated.

### Fixed

- fixed `TestWriteGoUpgradeCommands` failing on platforms without `/bin/bash`, such as Termux. The generated script execs the stub `go` binary by path, so the kernel resolved the stub's hard-coded `#!/bin/bash` shebang and failed with `bad interpreter`; the stub now names the bash the test already looked up with `exec.LookPath`, the same way the `internal/support` guard tests build theirs
- stopped selecting pre-releases and cross-major upgrades. Two ecosystems were taking a version the feed called newest without checking it was a finished release, or one the repository could survive. Maven has no notion of a pre-release -- `3.0.0-beta3`, `7.1.0-M1`, `5.5-beta2` and `5.7-alpha1` are ordinary releases to it, `allowSnapshots=false` excludes none of them, and `maven-metadata.xml` reports the newest of them as `<release>` -- so a single run moved four security pins onto pre-releases at once, and the log4j2 one dropped `log4j-api` back to a transitive 2.24.1 and reintroduced the three CVEs the pin existed to remediate. Both Maven goals now pass `-Dmaven.version.ignore` and `-DallowMajorUpdates=false`. Go crossed a major instead: `go get -u` will not reach for a `/v2` path, but v0 and v1 share the unsuffixed one, so an indirect `gobwas/glob` went v0.2.3 to v1.0.0, the major removed the `Glob` interface every published `gocolly/colly` still declares, and the module stopped compiling -- inside a pull request whose title claimed a routine update. The generated script now snapshots the requirements before the upgrade and holds anything whose major moved, keeping every other upgrade in the run rather than abandoning it. The pre-release vocabulary is defined once, in `internal/support/prerelease.go`, so the Go side and the Maven regexes cannot disagree.
- wired `allow_major_updates` into the ecosystems that were accepting it and ignoring it. It reached `java` and `golang` when it was introduced; `dart`, `terraform`, `pipeline`, `csharp` and `ruby` took the key through the configuration layers and then dropped it. Dart was the worst of those because it did not merely ignore the setting, it did the opposite: `pub upgrade --major-versions` is precisely a request to raise constraints across majors, and it was passed unconditionally, so a repository committing `allow_major_updates: false` had its constraints raised anyway with nothing in the log saying so. The four version-comparing updaters now go through `support.AcceptsUpgrade`, which is `IsNewerVersion` plus the major question asked together, so an ecosystem cannot answer one and forget the other -- `terraform`, `pipeline`, `csharp` and `ruby` previously took every major unconditionally. `javascript`, `python` and the gem half of `ruby` are untouched: they delegate to a package manager that already resolves inside the declared ranges, so there is nothing for the key to gate, and making them cross majors would be a new capability rather than a wiring job. `dockerfile` is also untouched, for the opposite reason -- it refuses every major unconditionally, so honouring the key there means relaxing a rule that predates it, which is a behaviour decision rather than a gap. The README, the shipped config and CLAUDE.md now name exactly which updaters honour it and why the rest do not.
