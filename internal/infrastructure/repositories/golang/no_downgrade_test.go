...
func TestGoDirectiveIsNeverDowngraded(t *testing.T) {
	...
	t.Run("should keep the directive when the module targets a newer Go than the latest stable", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("should raise the directive when the module is behind the latest stable", func(t *testing.T) {
		t.Parallel()

		// given
		repoDir := t.TempDir()
		scriptrunner.WriteFile(t, repoDir, "go.mod", goMod("1.24.0"))

		// when
		output := runGoUpgradeBlock(t, repoDir, "1.26.6")

		// then
		assert.Contains(t, scriptrunner.ReadFile(t, repoDir, "go.mod"), "go 1.26.6")
		assert.Contains(t, output, "GO_VERSION_UPDATED=true")
	})

	t.Run("should not re-apply the target over a directive tidy left ahead of it", func(t *testing.T) {
		t.Parallel()

		// given — a dependency requiring a newer Go makes `go mod tidy` raise the
		...
		// then
		assert.NotContains(t, scriptrunner.ReadFile(t, repoDir, "go.mod"), "go 1.26.6")
	})
